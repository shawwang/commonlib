    ...
    public void popActivity(String activity) {
        if (mActivityStack != null) {
            for (int i = 0; i < mActivityStack.size(); i++) {
                LogUtil.d("remove current activity:" + mActivityStack.get(i).getClass().getSimpleName());
...
//                        remove.finish();
     ...
     */
    public void clear() {
//        for (Activity activity : mActivityStack) {//不能用该方法遍历删除集合中元素(并发修改异常)
//            popActivity(activity);
//            activity.finish();
//        }
